 ...
 * @return 聊天室详细信息
 */
+(GotyeOCRoom*)getRoomDetail:(GotyeOCChatTarget*)target forceRequest:(bool)forceRequest;
 ...
 */
+(status)reqGroupList:(unsigned)pageIndex;
...
/**
 * @brief 发送客服评价
 * @param score: 评价分数 1～5分
 * @param comment: 评价内容
 * @param sessionId: 会话Id
 * @return 是否成功
 * @see callback: GotyeDelegate::onReqJudgeCSO
 */
+(status)reqJudgeCSO:(int)score comment:(NSString *)comment sessionId:(int)sessionId;

/**
 * @brief 获取聊天室在线人数
 * @param room: 聊天室
 * @return 是否成功
 * @see callback: GotyeDelegate::onGetRoomsMemberCount
 */
+(status)GetRoomsMemberCount:(GotyeOCRoom*)room;

/**
 * @brief 获取客服组详情
 * @param groupId: 客服组Id
 * @return 是否成功
 * @see callback: GotyeDelegate::onGetCSGroupDetail
 */
+(status)GetCSGroupDetail:(int)groupId;
