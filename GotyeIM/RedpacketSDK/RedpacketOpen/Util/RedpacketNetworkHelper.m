...
//定义一个变量
static RedpacketNetworkHelper *helper = nil;

@implementation RedpacketNetworkHelper
...
+ (instancetype)shareHelper
{
    @synchronized(self) {
        if (!helper) {
            helper = [[RedpacketNetworkHelper alloc] init];
        }
        return helper;
    }
}
...
+ (NSString *)parseParams:(NSDictionary *)params
{
    ...
    while (key = [keyEnum nextObject]) {
        ...
        result = [result stringByAppendingString:keyValueFormat];
    }
