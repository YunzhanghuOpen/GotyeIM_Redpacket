...
- (id)init {
  self = [super init];
  if (self) {
    self.tabBarItem.image = [[UIImage imageNamed:@"tab_button_message"]
...
- (void)setTabBarItemIcon {
  NSInteger unreadCount = [GotyeOCAPI getTotalUnreadMessageCount] +
                          [GotyeOCAPI getUnreadNotifyCount];
...
- (void)onLogin:(GotyeStatusCode)code user:(GotyeOCUser *)user {
  if (code == GotyeStatusCodeOK || code == GotyeStatusCodeOfflineLoginOK ||
      code == GotyeStatusCodeReloginOK) {
    [self.tableView reloadData];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  GotyeMessageCell *cell =
      [tableView dequeueReusableCellWithIdentifier:MessageCellIdentifier];
  if (cell == nil) {
    cell = [[[NSBundle mainBundle] loadNibNamed:@"GotyeMessageCell"
                                          owner:self
  ...
  if (sessionList.count + notifyList.count > sessionList.count) {
    if (indexPath.row == 0) {
      ...
      cell.labelMessage.text =
          [NSString stringWithFormat:@"共收到%lu个通知",
                                     (unsigned long)notifyList.count];
      ...
      if (notifyCount > 0) {
        ...
        [cell.buttonNewCount
            setTitle:[NSString stringWithFormat:@"%d", notifyCount]
      ...
      if (newCount > 0) {
        cell.buttonNewCount.hidden = NO;
        [cell.buttonNewCount
            setTitle:newCount > 99 ? @"99+"
                                   : [NSString stringWithFormat:@"%d", newCount]
            forState:UIControlStateNormal];
      } else
        cell.buttonNewCount.hidden = YES;
      ...
      NSString *msgDate = [formatter
          stringFromDate:[NSDate
                             dateWithTimeIntervalSince1970:lastMessage.date]];
      ...
      case GotyeChatTargetTypeGroup: {
        GotyeOCGroup *group =
            [GotyeOCAPI getGroupDetail:target forceRequest:NO];
        GotyeOCUser *user =
            [GotyeOCAPI getUserDetail:lastMessage.sender forceRequest:NO];
        ...
        //            cell.imageHead.image = [UIImage
        //            imageNamed:@"head_icon_group"];
        //            cell.labelMessage.text = [NSString
        //            stringWithFormat:@"%d位好友邀请你群聊",
        //            messageArray.count];
        //        }
        break;
    ...
    [formatter setDateFormat:@"MM-dd HH:mm"];
    NSString *msgDate = [formatter
        stringFromDate:[NSDate dateWithTimeIntervalSince1970:lastMessage.date]];
    ...
    case GotyeChatTargetTypeGroup: {
      ...
      //            imageNamed:@"head_icon_group"];
      //            cell.labelMessage.text = [NSString
      //            stringWithFormat:@"%d位好友邀请你群聊", messageArray.count];
      //        }
      break;
...
- (void)tableView:(UITableView *)tableView
  ...
  if (sessionList.count + notifyList.count > sessionList.count) {
    if (indexPath.row == 0) {
      ...
      GotyeNotifyController *viewController =
          [[GotyeNotifyController alloc] init];
...
- (UIButton *)contextMenuCell:(GotyeContextMenuCell *)cell
                buttonAtIndex:(NSUInteger)index {
  GotyeMessageCell *msgCell = [cell isKindOfClass:[GotyeMessageCell class]]
                                  ? (GotyeMessageCell *)cell
                                  : nil;
  switch (index) {
  case 0:
    return msgCell.buttonDelete;
  default:
    return nil;
  }
}
...
- (GotyeContextMenuCellButtonVerticalAlignmentMode)
          contextMenuCell:(GotyeContextMenuCell *)cell
alignmentForButtonAtIndex:(NSUInteger)index {
  return GotyeContextMenuCellButtonVerticalAlignmentModeCenter;
}

#pragma mark * GotyeContextMenuCell delegate

- (void)contextMenuCell:(GotyeContextMenuCell *)cell
    buttonTappedAtIndex:(NSUInteger)index {
  switch (index) {
  case 0: {
