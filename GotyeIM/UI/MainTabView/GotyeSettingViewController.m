...
@interface GotyeSettingViewController () <
    GotyeOCDelegate, UIActionSheetDelegate, UINavigationControllerDelegate,
...
- (id)init {
  self = [super init];
  if (self) {
    self.tabBarItem.image = [[UIImage imageNamed:@"tab_button_settings"]
...
- (void)viewDidLoad {
  ...
  globalSetting = [[GotyeSettingManager defaultManager]
      getSetting:[GotyeOCAPI getLoginUser].type
        targetID:[NSString
                     stringWithFormat:@"%lld", [GotyeOCAPI getLoginUser].id]];
}
...
didFinishPickingMediaWithInfo:(NSDictionary *)info {
  ...
  if (image.size.width > HeadMaxSize)
    image = [GotyeUIUtil scaleImage:image
                             toSize:CGSizeMake(HeadMaxSize, HeadMaxSize)];

  NSData *jpgTempImage =
      [GotyeUIUtil ConpressImageToJPEGData:image maxSize:ImageFileSizeMax];
  NSString *tempPath =
      [[GotyeSettingManager defaultManager]
...
- (void)willPresentActionSheet:(UIActionSheet *)actionSheet {
  for (UIView *subView in actionSheet.subviews) {
    if ([subView isKindOfClass:[UIButton class]]) {
      ...
      //            button.frame = CGRectMake(20, button.frame.origin.y, 280,
      ...
      //                imageNamed:@"button_gray"]
      //                forState:UIControlStateNormal];
      //                [button setBackgroundImage:[UIImage
      //                imageNamed:@"button_gray_down"]
      //                forState:UIControlStateHighlighted];
      //            }
      //            else
      //            {
      //                [button setBackgroundImage:[UIImage
      //                imageNamed:@"button_red"]
      //                forState:UIControlStateNormal];
      //                [button setBackgroundImage:[UIImage
      //                imageNamed:@"button_red_down"]
      //                forState:UIControlStateHighlighted];
      //            }
...
- (void)swichChanged:(UISwitch *)sw {
  ...
  [[GotyeSettingManager defaultManager]
      saveSetting:globalSetting
       targetType:[GotyeOCAPI getLoginUser].type
         targetID:[NSString
                      stringWithFormat:@"%lld", [GotyeOCAPI getLoginUser].id]];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  UITableViewCell *cell =
      [tableView dequeueReusableCellWithIdentifier:GotyeSettingCellIdentifier];
  if (cell == nil) {
  ...
  if (indexPath.section == 0) {
    ...
    case SettingUserTypeHead: {
      titleStr = @"头像";
      UIButton *headview = [[UIButton alloc]
          initWithFrame:CGRectMake(ScreenWidth - 20 - 46, 7, 46, 46)];

      GotyeOCUser *loginUser = [GotyeOCAPI getLoginUser];
      UIImage *headImage = [GotyeUIUtil getHeadImage:loginUser.icon.path
                                         defaultIcon:@"head_icon_user"];
      [headview setBackgroundImage:headImage forState:UIControlStateNormal];

      [headview addTarget:self
                    action:@selector(headSelectClick:)
    ...
    case SettingUserTypeNickName: {
      ...
      UILabel *contentText = [[UILabel alloc]
          initWithFrame:CGRectMake(ScreenWidth - 20 - 150, 15, 150, 20)];
  ...
  //        initWithFrame:CGRectMake(300 - 49, 10, 49, 31)];
  //
  ...
  //        {
  //            titleStr = @"群消息勿扰";
  //            switchView.on = [globalSetting[Setting_key_NoDisturb]
  //            boolValue];
  //        }
  //        [switchView addTarget:self action:@selector(swichChanged:)
  //        forControlEvents:UIControlEventValueChanged];
  //
  //        contentView = switchView;
  //    }
  else if (indexPath.section == 1) {
    UIButton *quitButton = [[UIButton alloc]
        initWithFrame:CGRectMake(20, 8, ScreenWidth - 40, 33)];
    [quitButton setBackgroundImage:[UIImage imageNamed:@"button_red"]
                          forState:UIControlStateNormal];
    [quitButton setBackgroundImage:[UIImage imageNamed:@"button_red_down"]
                          forState:UIControlStateHighlighted];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (indexPath.section == 0 && indexPath.row == SettingUserTypeNickName) {
    GotyeNickNameInputController *viewController =
        [[GotyeNickNameInputController alloc] init];
...
- (void)viewDidLoad {
  ...
  UIView *whiteBack =
      [[UIView alloc] initWithFrame:CGRectMake(0, 0, ScreenWidth, 55)];
  whiteBack.backgroundColor = [UIColor whiteColor];
  [self.view addSubview:whiteBack];

  textInput = [[UITextField alloc]
      initWithFrame:CGRectMake(18, 10, ScreenWidth - 36, 35)];
  ...
  [textInput addTarget:self
                action:@selector(textInputEnd:)
