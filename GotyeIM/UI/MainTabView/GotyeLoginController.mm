...
@synthesize contentView, textAppkey, textPort, textServer, appkeyTable,
    serverTable, serverView;
...
- (IBAction)onBtnLoginClick:(id)sender {
  if (textUsername.text.length > 0) {
    ...
    if ([GotyeOCAPI login:textUsername.text password:password] ==
        GotyeStatusCodeWaitingCallback)
...
- (IBAction)addServerClick:(id)sender {
  if (textServer.text.length > 0 && textPort.text.length > 0) {
    NSString *serverStr =
        [NSString stringWithFormat:@"%@:%@", textServer.text, textPort.text];
...
- (IBAction)saveSettingClick:(id)sender {
  ...
  // if(textServer.text.length > 0 && textPort.text.length > 0)
  {
    ...
    set_login_config(
        [textServer.text cStringUsingEncoding:NSUTF8StringEncoding],
        textPort.text.intValue);
  }
  ...
  [contentView scrollRectToVisible:CGRectMake(0, 0, ScreenWidth, 480)
                          animated:YES];
}
...
- (void)onLogin:(GotyeStatusCode)code user:(GotyeOCUser *)user {
  ...
  if (code == GotyeStatusCodeOK || code == GotyeStatusCodeOfflineLoginOK ||
    ...
    [[GotyeSettingManager defaultManager]
        setLoginUserName:[GotyeOCAPI getLoginUser].name];
...
- (void)viewDidLoad {
  [super viewDidLoad];
  appkeyPasteboard =
      [UIPasteboard pasteboardWithName:AppkeyRecentKey create:YES];
  appkeyPasteboard.persistent = YES;

  serverPasteboard =
      [UIPasteboard pasteboardWithName:ServerRecentKey create:YES];
...
- (void)viewWillAppear:(BOOL)animated {
  contentView.contentSize =
      CGSizeMake(ScreenWidth * 2, contentView.frame.size.height);
  [contentView addSubview:serverView];
  serverView.frame =
      CGRectMake(contentView.frame.size.width, 0, serverView.frame.size.width,
                 serverView.frame.size.height);
  ...
  if (![recentAppkeys
          containsObject:[userDefaults objectForKey:AppkeyDefaultKey]]) {
  ...
  if (index < recentAppkeys.count) {
    [appkeyTable
        selectRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]
                    animated:NO
...
- (void)viewDidAppear:(BOOL)animated {
  ...
  if (autologin) {
    ...
    [self performSelector:@selector(onBtnLoginClick:)
               withObject:nil
               afterDelay:0.2];
  }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  UITableViewCell *cell =
      [tableView dequeueReusableCellWithIdentifier:AppKeyCellIdentifier];
  if (cell == nil) {
    cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
                                  reuseIdentifier:AppKeyCellIdentifier];
