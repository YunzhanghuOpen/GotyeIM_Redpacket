...
#define DEFAULT_APPKEY                                                         \
  (@"2520e28f-7cb6-4c47-90bf-da1f854d294d") //*/(@"9c236035-2bf4-40b0-bfbf-e8b6dec54928")
...
- (BOOL)application:(UIApplication *)application
  ...
  self.viewController = [[GotyeTabViewController alloc]
      initWithNibName:@"GotyeTabViewController.xib"
               bundle:nil];
  ...
  if (selectedAppKey != nil && selectedAppKey.length > 0) {
    NSString *serverAddr =
        [serverStr substringToIndex:[serverStr rangeOfString:@":"].location];
    NSString *serverPort = [serverStr
        substringFromIndex:[serverStr rangeOfString:@":"].location + 1];
    set_login_config([serverAddr cStringUsingEncoding:NSUTF8StringEncoding],
                     serverPort.integerValue);
  }
...
- (void)application:(UIApplication *)application
  ...
  //[GotyeAPI registerRemoteNotificationWithDeviceToken:deviceToken
  // certName:certName];
}
